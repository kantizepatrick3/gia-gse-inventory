...
async function updateSchema() {
  try {
    console.log('🔧 Adding timestamp columns without defaults...\n');
    
    // Add created_at column without default
    try {
      await db.execute(`ALTER TABLE parts ADD COLUMN created_at DATETIME`);
    ...
    try {
      await db.execute(`ALTER TABLE parts ADD COLUMN updated_at DATETIME`);
